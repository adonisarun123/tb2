...
import { supabase, isSupabaseAvailable } from './supabaseClient.ts';
...
const fetchDynamicRoutes = async (): Promise<SitemapURL[]> => {
  ...
  if (!isSupabaseAvailable || !supabase) {
    console.warn('⚠️  Skipping dynamic routes - Supabase not available');
  ...
  trainings?.forEach(training => {
    ...
    console.error('⚠️  Error fetching dynamic routes from Supabase:', error);
    console.warn('⚠️  Continuing with static routes only');
  }
...
const generateSitemap = async (): Promise<string> => {
  ...
  console.log(`📊 Generated sitemap with ${staticRoutes.length} static routes and ${dynamicRoutes.length} dynamic routes (${allRoutes.length} total)`);
